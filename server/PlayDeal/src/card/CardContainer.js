const { getIdFromObj } = require("../utils/objectMethods");
...
module.exports = class CardContainer {
    ...
    findCards(fn) {
        if (typeof fn === "string") {
            return this.findCardsWithKey(fn);
        }
    ...
    findCardWithKey(cardKey) {
        return this.findCard((card) => card.getKey() === cardKey);
    }

    findCardsWithKey(cardKey) {
        return this.findCards((card) => card.getKey() === cardKey);
    }
